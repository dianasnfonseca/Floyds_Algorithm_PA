...
"""
The solution that is documented here creates a graph matrix. The solution matrix is then
updated by assuming all nodes are an intermediate node. The solution picks a node and
then computes the distance between every node that includes the intermediate node as part
of the path. It checks if the path is the current shortest path, if it is the graph is updated and if
not the value is ignored.
"""
...
"""
floyd is the main function in the code below. We use product to create the
